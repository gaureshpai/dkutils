...
const JsonXmlConverter = () => {
	...
	const convertJsonToXml = async () => {
		...
		try {
			...
			setError(
				err.response?.data?.msg || "Failed to convert JSON to XML. Please check your input.",
			);
	...
	const convertXmlToJson = async () => {
		...
		try {
			...
			setError(
				err.response?.data?.msg || "Failed to convert XML to JSON. Please check your input.",
			);
		...
		<div className="container mx-auto p-4">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div className="p-4 border rounded-md bg-background shadow">
					<label htmlFor="inputData" className="block mb-2 text-sm font-medium text-foreground">
				...
				<div className="p-4 border rounded-md bg-background shadow">
					<label htmlFor="outputData" className="block mb-2 text-sm font-medium text-foreground">
						Output
						{outputData && (
							<button
								type="button"
			...
			{error && (
				<div className="mt-4 p-3 bg-destructive/10 text-destructive rounded-md">{error}</div>
			)}
		</div>
	);
};
