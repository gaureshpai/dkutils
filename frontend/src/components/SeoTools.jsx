...
const SeoTools = () => {
	...
	const fetchRobotsTxt = async () => {
		...
		try {
			const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/seo/robots-txt`, {
				domain,
			});
			if (res.data.exists) {
				...
				setRobotsTxtError(res.data.error || "robots.txt not found or accessible.");
				setRobotsTxtContent("");
	...
	const fetchSitemapXml = async () => {
		...
		try {
			const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/seo/sitemap-xml`, {
				domain,
			});
			if (res.data.exists) {
	...
	return (
		<div className="container mx-auto p-4">
			...
			<div className="mb-4">
				<label htmlFor="domainInput" className="block mb-2 text-sm font-medium text-foreground">
