...
const UrlRedirectChecker = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/redirect-checker`, {
				url,
			});
			...
			setError(
				err.response?.data?.msg || "Failed to check redirects. Please check the URL and try again.",
			);
	...
	return (
		<div className="container mx-auto p-4">
			...
			<form onSubmit={handleSubmit}>
				<div className="mb-4">
					<label htmlFor="urlInput" className="block mb-2 text-sm font-medium text-foreground">
						URL to Check
					</label>
					<input
						type="url"
